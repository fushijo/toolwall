...
pub fn show(ui: &mut egui::Ui, doc: &mut Document) {
    egui::ScrollArea::vertical().show(ui, |ui| {
        ...
        egui::Grid::new("theme-grid")
            ...
            .spacing([12.0, 6.0])
            .show(ui, |ui| {
                ui.label("background").on_hover_text("#rrggbb or #rrggbbaa");
        ...
        ui.separator();
        ui.heading("Floating window position");
        ui.weak(
            "waywall anchors whichever floating window is the only one open, so this \
             positions Ninjabrain Bot when it is running alone. Unanchored windows sit at \
             the top-left and can be moved with Shift + drag.",
        );
        ...
        ui.weak("Applies immediately. Saved with the rest of your config.");
